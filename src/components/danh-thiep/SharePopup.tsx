...
import { effectiveOrigin, linkDanhThiep, useSiteOrigin } from "./lib";
...
export function useChiaSe(a: Advisor) {
  ...
  const chiaSe = async (nut: NutChiaSe) => {
    ...
    if (nut === "Zalo") { flash("Đã mở Zalo để gửi danh thiếp"); window.open(`https://zalo.me/share?url=${encodeURIComponent(linkDanhThiep(a.ma, "zalo", o))}`, "_blank", "noopener"); return; }
    if (nut === "Facebook") { flash("Đã mở Facebook để chia sẻ"); window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(linkDanhThiep(a.ma, "fb", o))}`, "_blank", "noopener"); return; }
    if (nut === "Sao chép link") {
      try { await navigator.clipboard.writeText(linkDanhThiep(a.ma, "copy", o)); flash("Đã sao chép liên kết danh thiếp"); } catch { flash(linkDanhThiep(a.ma, "copy", o)); }
      return;
    }
...
function taiAnhThe(a: Advisor) {
  ...
  g.fillStyle = "#808080"; g.font = "24px Arial"; g.fillText(linkDanhThiep(a.ma, "qr", effectiveOrigin()), 80, 560);
      ...
      <Modal open={open} onClose={onClose} title="Chia sẻ danh thiếp" width={560}>
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 text-[12.5px] text-mut break-all">{linkDanhThiep(a.ma, undefined, origin)}</div>
          </div>
        </div>
      </Modal>
      {flashNode}
