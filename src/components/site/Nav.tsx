 ...
 * A01 dùng chung cho khách và TVV (quyết định 08/09, bỏ A02).
 */
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-vien2">
      <div className="wrap h-16 flex items-center gap-6">
        <Link href={R.A01} className="flex items-center gap-1 shrink-0" aria-label="Chubb Life — Trang chủ">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/chubb.svg" alt="Chubb" className="h-5" />
        </Link>
        <nav className="hidden lg:flex items-center gap-5 text-[14px] font-bold" aria-label="Chính">
          <Link href={R.A01} className={cx("hover:text-blue", activeCode === "A01" ? "text-blue" : "text-ink2")}>Trang chủ</Link>
          {TABS.map((t) => (
            <Link key={t.code} href={t.href} className={cx("hover:text-blue", activeCode === t.code ? "text-blue" : "text-ink2")}>{t.label}</Link>
          ))}
        </nav>
        <form className="ml-auto hidden md:block" onSubmit={(e) => { e.preventDefault(); if (q.trim()) router.push(R.S01(q.trim())); }}>
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm kiếm toàn site…" aria-label="Tìm kiếm toàn site" className="h-9 w-[200px] rounded-sm border border-vien px-3 text-[13px] focus:outline-none focus:border-blue" />
        </form>
        {!tvv ? (
          ...
          <div className="relative flex items-center gap-3" ref={ref}>
            ...
            <button type="button" onClick={() => setMenu(menu === "account" ? "none" : "account")} className="flex items-center gap-2 h-9 pl-1 pr-3 rounded-sm border border-vien hover:border-blue text-[13px] font-bold text-den">
              <Avatar name={tvv.hoTen} size={28} /> <span className="hidden md:inline">{tvv.hoTen}</span> <span aria-hidden className="text-mut">▾</span>
            </button>
            {menu === "account" && (
